...
public static partial class UtilMath
{
    ...
    [MethodImpl(INLINE)] public static float LerpClamped(float from, float to, float weight) => Clamp01(from + (to - from) * weight);

    [MethodImpl(INLINE)] public static float InverseLerp(float from, float to, float weight) => (weight - from) / (to - from);
    [MethodImpl(INLINE)] public static float InverseLerpClamped(float from, float to, float value) => Clamp01((value - from) / (to - from));
    ...
    [MethodImpl(INLINE)] public static float AddToAverage(float average, float newValue, int newSize) => average + (newValue - average) / newSize;

    [MethodImpl(INLINE)] public static float SubtractFromAverage(float average, float newValue, int newSize) => average + (newValue - average) / newSize;

    [MethodImpl(INLINE)] public static float ReplaceInAverage(float average, int size, float oldValue, float newValue) => (size * average - oldValue + newValue) / size;

    [MethodImpl(INLINE)] public static float MergeAverages(float average1, int size1, float average2, int size2) => (size1 * average1 + size2 * average2) / (size1 + size2);
